...
import com.liuzemin.server.framework.model.annotation.JSZLJSONS;
...
@Component
public class JsonMethodReturnValueHandler implements HandlerMethodReturnValueHandler, BeanPostProcessor {

	List<ResponseBodyAdvice<Object>> advices = new ArrayList<>();

	@Override
	public boolean supportsReturnType(MethodParameter returnType) {
		boolean hasJSONAnno = returnType.getMethodAnnotation(JSZLJSON.class) != null || returnType.getMethodAnnotation(JSZLJSONS.class) != null;
		return hasJSONAnno;
	}
	...
	public void handleReturnValue(Object returnValue, MethodParameter returnType, ModelAndViewContainer mavContainer, NativeWebRequest webRequest)
		...
		Arrays.asList(annos).forEach(a -> {
			if (a instanceof JSZLJSON) {
				JSZLJSON json = (JSZLJSON) a;
				jsonSerializer.filter(json);
			} else if (a instanceof JSZLJSONS) {
				JSZLJSONS jsons = (JSZLJSONS) a;
	...
	public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
		if (bean instanceof ResponseBodyAdvice) {
			...
			JsonMethodReturnValueHandler jsonHandler = null;
			for (int i = 0; i < handlers.size(); i++) {
				HandlerMethodReturnValueHandler handler = handlers.get(i);
				if (handler instanceof JsonMethodReturnValueHandler) {
					jsonHandler = (JsonMethodReturnValueHandler) handler;
					break;
				}
			}
			if (jsonHandler != null) {
